...
function VerificationContent() {
  ...
  const [phone, setPhone] = useState<Phone>({ 0: '', 1: '', 2: '', 3: '' });
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);

  const isCodeComplete = Object.values(phone).every(value => value !== '');
  ...
  const handleSubmitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const code = Number(Object.values(phone).join(''));
      const { accesToken, refreshToken } = await loginEndpoint(code);
      ...
      <div className="w-full max-w-md mx-auto px-6">
        ...
        <form className="space-y-8" onSubmit={handleSubmitForm}>
          <div className="flex justify-between gap-4">
            {[1, 2, 3, 4].map(i => (
              <input
                ...
                type="text"
                maxLength={1}
                value={phone[i - 1 as keyof Phone]}
          ...
          <button
            ...
            className="w-full h-14 rounded-lg bg-black text-white text-lg font-medium flex items-center justify-center gap-2"
          >
            <span>Verificar</span>
